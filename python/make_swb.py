...
WEATHER_DATA_LOOKUP_TABLE 1944-01_to_2023_02_3037835_kenai_airport.txt
...
OUTPUT DISABLE runoff actual_et rainfall snowmelt 
...
def define_swb_batch_file(filename):
    result = f"""
swb2 --output_prefix=swb_test__       ^
     --data_dir=input                 ^
     --weather_data_dir=input         ^
     --lookup_dir=input               ^
     --output_dir=output              ^
     --logfile_dir=output             ^
        {filename}
...
def write_swb_control_file(filename):

    swb_control_file_text = define_swb_control_file()

    with open(filename, 'w') as f:
        f.write(swb_control_file_text)


def write_swb_batch_file(swb_control_filename):

    swb_batch_file_text = define_swb_batch_file(swb_control_filename)

    with open('run_swb.bat', 'w') as f:
        f.write(swb_batch_file_text)        